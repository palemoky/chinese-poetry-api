...
func TestGetPoetryTypePoems(t *testing.T) {
	router, repo := setupPoetryTypeTestRouter(t)
	handler := NewPoetryTypeHandler(repo)

	router.GET("/types/:id/poems", handler.GetPoetryTypePoems)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/types/"+tt.typeID+"/poems"+tt.query, nil)
			w := httptest.NewRecorder()
